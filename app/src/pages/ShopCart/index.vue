      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(item,index) in cartInfoList" :key="item.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="item.isChecked==1" @click="updateChecked(item,$event)">
          </li>
    ...
    methods: {
     ...
     handler:throttle( async function (type,disNum,cart) {
      switch (type) {
        ...
        try {
          //切记结构参数名匹配！
          await this.$store.dispatch('shopCart',{skuId:cart.skuId,skuCount:disNum})
          this.getData()
        } catch (error) {
          console.log(error);
        }
    ...
    computed:{
      ...
      sum(){
        ...
        this.cartInfoList.forEach(element => {
          count += element.skuPrice*element.skuNum
        });
        return count
      },
      //判断底部复选框是否勾选
